...
===============================================================================================*/
      ...
      exec(_.set("env", s"${env}")
        ...
        .repeat(1) {
          feed(c100RespondentData)
            ...
            .exec(Citizen_PRL_C100_Respondent.ConfirmEditContactDetails)
            // New for R6.0
      ...
      exec(_.set("env", s"${env}")
        ...
        .repeat(1) {
          feed(fl401ApplicantDashData)
            ...
            .exec(Citizen_PRL_FL401_ApplicantDashboard.ConfirmEditContactDetails)
            // New for R6.0
          ...
          exec(_.setAll(
            ...
            .exec(
              ...
              CCDAPI.CreateEvent("CourtAdmin", "PRIVATELAW", "PRLAPPS", "serviceOfApplication", "bodies/prl/courtAdmin/PRLSoASubmit.json",
                Seq(
                  jsonPath("$.case_data.caseInvites[0].value.accessCode").saveAs("prlAccessCodeApplicant"),
                  jsonPath("$.case_data.caseInvites[2].value.accessCode").saveAs("prlAccessCodeRespondent"))),
              Logout.XUILogout)
            //================================
            //Request and List Hearings x 2
            //================================
  ...
  * Simulation Configuration
  ...
  def assertions(testType: String): Seq[Assertion] = {
    testType match {
      case "perftest" => //currently using the same assertions for a performance test and the pipeline
        if (debugMode == "off") {
          Seq(global.successfulRequests.percent.gte(95),
            ...
            details("XUI_PRL_XXX_580_OrderSubmit").successfulRequests.percent.gte(80),
  ...
  def setUpType(testType: String): List[PopulationBuilder] = {
    testType match {
      case "perftest" =>
        List(
      ...
      case "dataprep" =>
        List(
          PRLC100CreateProgressCase.inject(rampUsers(250).during(60.minutes)),
          PRLFL401CreateProgressCase.inject(rampUsers(150).during(60.minutes)))
